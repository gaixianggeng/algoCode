...
//leetcode 3 ÎÞÖØ¸´×Ö·ûµÄ×î³¤×Ó´®
//½¨Ò»¸ömap´æ´¢Ã¿¸ö×Ö·û£¬endIndexÀÛ¼Ó
//Èçkey´æÔÚµÄ»° ¸úÐÂlongest startIndexÍ¬Ê±¼ÓÒ»
...
//15ÈýÊýÖ®ºÍ
//Á½ÊýÖ®ºÍµÄË¼Â·Ì×ÔÚÕâÀï ÐèÒªÈ¥ÖØ  ËùÒÔ²ÉÓÃ×óÓÒ±ê³ß·½Ê½
...
void quickSort(vector<int>& arr, int left, int right)
{
	...
	arr[left] = arr[i];
	arr[i] = base;
	quickSort(arr, left, i - 1);//µÝ¹é×ó±ß
	quickSort(arr, i + 1, right);//µÝ¹éÓÒ±ß
}
...
//×î³¤Á¬ÐøÐòÁÐ
//Í¨¹ýmap ±éÀúnums¡£¼Ù¶¨Ã¿¸öitem¶¼ÊÇµÚÒ»¸ö ÅÐ¶Ï+1»ò-1ÊÇ·ñ´æÔÚmapÀïÃæ£¬´æÔÚµÄ»° ¸üÐÂ³¤¶È¡£×îºóÈ¡×î´óÖµ¡£
...
int longestConsecutive(vector<int>& nums) {
	...
	for (int i = 0; i < nums.size(); i++) {
		int l = 1;
		int temp = nums[i];
		//Õâ¸öifÅÐ¶ÏÊÇ¿ØÖÆÊ±¼ä¸´ÔÓ¶ÈµÄ¹Ø¼ü ¼ÙÉèÈ¡µ½µÄ¶¼ÊÇ×îÐ¡µÄ
...
//µÚk¸öÅÅÁÐ
//³õ²½Ïë·¨ µÝ¹é ÒÀ´ÎÈ·¶¨
//ÏÈÊµÏÖn¸öÊý×ÖÒÀ´Î´òÓ¡ ÔÙÕÒ¶ÔÓ¦kµÄÊý×Ö
